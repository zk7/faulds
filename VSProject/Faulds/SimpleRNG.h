/*
SimpleRNG from https://www.johndcook.com/blog/cpp_random_number_generation/
*/
#include <cmath>
#include <stdexcept>
#include <sstream>
...
public:
	...
	// Get reverse exponential upto right side limit
	double GetReverseExponential(double mean, double limit) {
		if (mean <= 0.0)
		{
			std::stringstream os;
			os << "Exponential mean must be positive." << "\n"
		...
		double r = -1;
		while (r < 0) r = limit - GetExponential(mean); //make sure to generate positive number

		return r;
	}

	double GetErlang(double mean, double k) {
		if (mean <= 0.0)
		{
			std::stringstream os;
			os << "Exponential mean must be positive." << "\n"
